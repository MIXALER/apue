...
![image-20210429103909174](assets/image-20210429103909174.png)
...
![image-20210429165828830](assets/image-20210429165828830.png)
...
-   **down** : 如果信号量大于 0 ，执行 -1 操作；如果信号量小于 0，则该进程被阻塞后进入该信号相对应的队列中，然后转进程调度；若信号量仍然大于等于 0，则进程继续进行。
-   **up** ：对信号量执行 +1 操作，若相加的结果小于等于 0，则从该信号的等待队列中唤醒咦等待进程，然后再返回源进程执行或转进程调度。若相加结果大于零，则进程继续执行。那么，信号量小于 0 时，其绝对值表示等待该信号量的进程数量。

down 和 up 操作需要被设计成原语，对于每个进程来说，都只能进行一次。而且必须成对使用。通常的做法是在执行这些操作的时候屏蔽中断。具体如何屏蔽中断由操作系统完成，对用户透明。
...
https://www.cnmooc.org/portal/session/bulletin/index/14956.mooc